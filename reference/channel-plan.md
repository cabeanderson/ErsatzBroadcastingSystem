...
**Serialized shows stay off the bench.** Gravity Falls, The Owl House and Amphibia carry the chronological-weekday / shuffle-weekend split in the 17:00–19:00 strip, and the shuffle half runs again at 00:00–02:00 as Disney After Dark — same two keys, no third registration. Star Wars (Clone Wars 133, Rebels 69, Bad Batch 47) runs chronologically at 19:00–21:00, with the shuffled vault behind it at 21:00–24:00.

**Star Wars is the one strip that does *not* carry the weekday/weekend split, and it took a checker to notice — 2026-09-09.** It was written as one: `STAR_WARS_STRIP = {"WEEKDAY": first-run, "default": reruns}`, with the library comment promising "shuffled at the weekend". The weekend never came. The only path to that dict is `PRIME_BLOCK`'s own `default`, and Saturday and Sunday are routed to STAR_WARS_EVENT_NIGHT and WONDERFUL_WORLD_OF_DISNEY before it is consulted — so it is reached Mon–Fri only, where the `WEEKDAY` label always wins and the reruns branch was unreachable on all seven days. `STAR_WARS_RERUNS`, a named `Block` holding three real content keys, could never air. Removed: the shuffled weekend it described is what the vault already does at 21:00–24:00, and `star_wars_animation_tv` contains all three shows. A two-year diff of the channel reads 0 of 44,163 airings changed. Found by [`testing/unaired_check.py`](../testing/unaired_check.py) on its first run.
...
**Prime is seven different pools and the mystery shelf is not one of them.** Poirot, Marple and Morse own 09:00–12:00 on weekdays. Running them at eight as well would be one pool wearing two blocks (G4) *and* would collide with Mystery Theatre on four nights of seven. Instead: Monday gritty (Peaky, Gangs of London), Tuesday thriller (Killing Eve, The Terror, Years and Years), Wednesday after dark (Luther, Black Mirror, I May Destroy You), Thursday period (**Sharpe**, over a British period film), Friday sketch (Python, Mitchell and Webb, Serafinowicz), Saturday film, Sunday Attenborough.

**Sharpe is a season, not a title in a rotation — added 2026-09-09.** Sixteen feature-length films shared a shuffled three-title Thursday with A Young Doctor's Notebook and Queer as Folk, so Sharpe surfaced about every third week in no order, took a year to cycle, and never read as a run. It is an `annual_show` now: the seven on-disk seasons chained into **one unbroken sixteen-week run every spring**, one film a Thursday at 20:00, opening on *Sharpe's Rifles*. The date is not arbitrary — sixteen Thursdays from SPRING's 15 March peak puts **Sharpe's Waterloo, the fourteenth film, in the week of 18 June** every year from 2027 to 2032, and on the eve itself in two of them. Spring is also the only clear window: the channel has both holiday and seasonal injection on, so an autumn run would take Halloween through its middle and lose a Thursday to the DOCTOR_WHO_DAY schedule. **18 June is a marathon** — Revenge, Justice, Waterloo, then the 1970 film — and it can be one because only Halloween, Thanksgiving and Christmas set `is_holiday_season` (G9), which is exactly why Doctor Who Day could not.

**The Thursday bed is a British period film, and it had to come off the film shelf.** An appointment needs its own night *and its own bed* (G5); taking Sharpe out left 21 episodes and 9.1 hours against ~108 hours of off-season Thursday prime, a twelvefold replay that fails F6. Every drama shelf on the channel is spoken for on another night, and Doc Martin is what keeps The Lunch Break above F6 — so `BRITISH_PERIOD_FILM` is 21 newly named titles in Sharpe's own register, Waterloo and Master and Commander either side of the run. None is named on any other channel: `same_title_check` and `collision_report` both read identical before and after. The block cannot wrap and replay Sharpe twice a night — the G5 trap — by arithmetic rather than luck: the shortest film is 101 minutes, so Sharpe plus one feature is at least 201 against a 180-minute prime.

Verified over three simulated years (2027–2029): 1,096 days, no circuit breakers, no unresolved programs, no missing content; 16 Thursdays plus the marathon each year; Sharpe never on another night and never twice in one.
