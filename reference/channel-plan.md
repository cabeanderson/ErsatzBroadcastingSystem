...
|---|---:|---:|---|
| **Disney** | 1,364 eps + ~31 films | 22 | Disney Afternoon core, Star Wars animation as its own strip |
...
### Disney
Disney morning cartoons, Disney Afternoon mid-afternoon, Sunday night movie. Star Wars animation (277 eps / 7 shows) works as a separate strip rather than mixed in.
...
**Collision report.** The framework has no cross-channel awareness. Sharing rules stay a convention until something can simulate N days across all channels and flag a title double-booked in the same hour. `visualize_week.py` and the walker in `validate_titles.py` are the two halves of it — same traversal, plus a time dimension. Scheduled after Disney and before the Cartoon Network restructure, where it earns its keep by producing CN's eviction list; see [next-session.md](next-session.md).
...
| **Features with zero uses** | `alternating_seasons()`, `timeslot_commercials`, `global_filter`, `seasonal_blocks` param, `loop_restart_season` |
